...
export async function summary(req: Request, res: Response) {
  try {
    ...
    sendSuccess(res, "Cash flow summary", {
      income,
      expense,
      net: income - expense,
      cashInHand,
    ...
    sendError(res, "Failed to fetch cash flow", e.message);
  }
}
  ...
  try {
    ...
    for (const t of transactions) {
      if ((t as any).amount <= 0) {
        alerts.push({ type: "invalid_amount", id: (t as any)._id });
      }
      // add more anomaly detection as needed
    }
    sendSuccess(res, "Cash flow anomalies", { alerts });
  } catch (e: any) {
    sendError(res, "Failed to fetch anomalies", e.message);
  }
}
