...
import { 
  ...
  rentInventoryItem,
  returnInventoryItem as returnInventoryItemService,
...
export const rentInventoryItemController = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { quantity, startDate, endDate, userId, notes } = req.body;
    ...
    const result = await rentInventoryItem(
      id,
      quantity,
      userId,
      notes || "Item rented"
...
export const returnInventoryItemController = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { rentalId, returnCondition, returnNotes, lateFee, damageFee, quantity, userId } = req.body;
    const companyId = req.user?.companyId;
    ...
    const result = await returnInventoryItemService(
      id,
      quantity || 1,
      userId,
...
export const getInventoryStatisticsController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await getInventoryStatistics(companyId);
    sendSuccess(res, "Inventory statistics fetched successfully", result);
  } catch (error) {
...
export const getLowStockItemsController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await getLowStockItemsService(companyId);
    sendSuccess(res, "Low stock items fetched successfully", result);
  } catch (error) {
...
export const getMaintenanceDueItemsController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await getMaintenanceDueItems(companyId);
    sendSuccess(res, "Maintenance due items fetched successfully", result);
  } catch (error) {
