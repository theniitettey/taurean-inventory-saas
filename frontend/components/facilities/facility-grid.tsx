...
import { FacilityCard, type Facility } from "./facility-card"
      ...
      <section className="py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center py-12">
            <h3 className="text-lg font-medium text-slate-900 mb-2">Failed to load facilities</h3>
    ...
    <section className="py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-6">
          <Link
            href={`/facilities?location=${encodeURIComponent(location)}`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {isLoading
            ? Array.from({ length: 8 }).map((_, index) => <FacilityCardSkeleton key={index} />)
            : facilities.map((facility) => (
                <FacilityCard key={facility.id} facility={facility} onFavoriteToggle={onFavoriteToggle} />
              ))}
        </div>
